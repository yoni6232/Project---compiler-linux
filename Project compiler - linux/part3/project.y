...
ASSIGNMENT:				ASSIGNMENT_VAR assign EXPR SEMI{ 
						...
						| BAND ASSIGNMENT_VAR assign EXPR SEMI{ 
						$$=mknode($3->token,mknode("&",$2,NULL,NULL,NULL),$4,NULL,NULL);
							strcat($$->code, $2->code);
							strcat($$->code,$4->code);
							strcat($$->code,"\t");
							strcat($$->code,$2->var);
						...
						| ASSIGNMENT_VAR assign BAND id OSQRERACKET EXPR CSQRERACKET SEMI{
							$$=mknode($2->token,$1,mknode("&",$4,NULL,mknode("SIZE",$6,NULL,NULL,NULL),NULL),NULL,NULL);
						...
						| MULT ASSIGNMENT_VAR assign EXPR SEMI{
...
RETURN_STATMENT:		RRETURN EXPR SEMI { 
							$$=mknode("RETURN",$2,NULL,NULL,NULL);
							strcat($$->code, "\n");
							strcat($$->code, $2->code);
							strcat($$->code, "\tRETURN ");
							strcat($$->code, $2->var);
						...
						| RRETURN SEMI { 
							$$=mknode("RETURN",NULL,NULL,NULL,NULL); 
			   			    strcat($$->code, "");
						}
						|
						;
...
EXPR_LIST:				EXPR_LIST COMMA EXPR { 
						$$=mknode("VARIABLES",$3,$1,NULL,NULL); 
						strcat($$->code,$1->code);
...
id:						ID 	{
							$$=mknode(yytext,NULL,NULL,NULL,NULL);
							strcpy($$->var, yytext);
						}
						;
